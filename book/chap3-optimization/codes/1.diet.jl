...
@constraint(model, 0 * x1 + 7 * x2 >= 8)    # Requirement for nutrient 1
@constraint(model, 4 * x1 + 2 * x2 >= 15)   # Requirement for nutrient 2
@constraint(model, 2 * x1 + x2 >= 3)     # Requirement for nutrient 3

# Set the objective function to minimize the total cost
# where x1 costs 6 units and x2 costs 3.5 units
@objective(model, Min, 6 * x1 + 3.5 * x2)
