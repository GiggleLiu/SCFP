...
- Remark 1: Flops measures the *arithmetic complexity* of the matrix multiplication more precisely than the time complexity.

== Can we do better? Strassen's algorithm
#timecounter(2)
...
- _Remark:_ There are 7 matrix-matrix products of size $n/2 times n/2$, which dominates the time complexity.
...
== Fast Fourier Transform - Another divide and conquer algorithm
#timecounter(2)
...
- _Remark:_ The Cholesky decomposition is a special case of LU decomposition where $L = U^T$. It can be computed in a similar way.
...
The above Gaussian elimination process (#link("https://github.com/GiggleLiu/ScientificComputingDemos/blob/3db85a9186d76fc0032b72bd08b71fe2171de3d5/SimpleLinearAlgebra/src/lu.jl#L220")[code]) is numerically unstable if $A$ has a diagonal entry that is close to zero.
```julia
            m[i, k] = a[i, k] / a[k, k]
```
...
- _Remark_: While complete pivoting provides superior numerical stability compared to partial pivoting, its implementation is more complex and _computationally expensive_. In practice, partial pivoting usually provides sufficient numerical stability for most applications while being simpler and faster to compute.

= Avoid Loss of Orthogonality

== Gram-Schmidt Orthogonalization
#timecounter(2)
To orthogonalize two vectors $x$ and $y$, we project out $x$ from $y$:
$
e_x = x\/||x||_2, quad y_bot = y - (y^dagger e_x)e_x
$

#figure(canvas({
  import draw: *
  let s(it) = text(12pt)[#it]
  line((0, 0), (2, 0), mark: (end: "straight"), name: "x")
  line((0, 0), (3, 2), mark: (end: "straight"), name: "y")
  line((0, 0), (3, 0), stroke: (dash: "dashed"), mark: (end: "straight"), name: "ypar")
  content((rel : (0.0, 0.2), to: "x.mid"), s[$x$])
  content((rel : (0.5, 0.1), to: "y.end"), s[$y$])
  content((rel : (0.0, -0.2), to: "ypar.mid"), s[$y_(parallel)$])
  line("y.end", "ypar.end", stroke: (dash: "dashed"), name: "e_x")
  line("ypar.end", "y.end", stroke: (dash: "dashed"), mark: (end: "straight"), name: "ybot")
  content((rel : (0.3, 0.0), to: "ybot.mid"), s[$y_bot$])
}))

The Gram-Schmidt orthogonalization is the simplest method to compute the QR factorization of a matrix $A = (a_1, dots, a_n) in RR^(m times n)$. For each column $k$, we:
...
#align(center, box(stroke: black, inset: 10pt, [*Problem*: Loss of orthogonality:
$
Q^dagger Q = I + E_"GS", quad ||E_"GS"||_2 = O(u kappa(A))
$
]))
...
https://github.com/GiggleLiu/ScientificComputingDemos/blob/3b73e6f1ec842ee9f02569c204d6740f022d6b0a/SimpleLinearAlgebra/src/qr.jl#L267

- _Remark_: To implement an array, the interfaces are:
  - `Base.getindex`
  - `Base.setindex!` (optional)
  - `Base.size`
  - `LinearAlgebra.mul!` (optional)
- _Remark_: The Householder reflection computes in $O(n^2)$ time.
// #columns(2, text(14pt, [```julia
...
=== Tasks:
1. Read the code demo: https://github.com/GiggleLiu/ScientificComputingDemos/tree/main/SimpleLinearAlgebra
2. Improve the Strassen's algorithm with the BLAS routines.
3. Benchmark the performance of the improved Strassen's algorithm with the existing BLAS routines for computing matrix-matrix products. Record the time, and predict the size where the performance gain is positive.
